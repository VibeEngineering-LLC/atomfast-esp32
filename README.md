...
<p align="center">
  <img src="images/atomfast-s3-n16r8-in-operation.jpg" alt="ESP32-S3-DevKitC-1 N16R8 (AtomFast-шлюз) с внешней U.FL-антенной — плата в работе" width="380">
  <br>
  <em>Рекомендованная плата в работе: ESP32-S3-DevKitC-1 N16R8 с внешней U.FL-антенной — приборный прогон ≈ 4 суток без ребутов (2026-06-25).</em>
</p>

## Три прошивки в скилле — какую выбрать

С `v0.9.0-c3` файлы прошивок разложены по индивидуальным подпапкам по типу платы:

```
firmware/
├── esp32-classic/         atomfast_gateway.yaml + include/gatt_dump.h
├── esp32-s3-devkitc/      atomfast_gateway_s3.yaml
├── esp32-c3-supermini/    atomfast_gateway_c3.yaml
├── secrets.example.yaml   (общий шаблон секретов)
├── CHANGELOG.md
├── archive/               (исторические версии классики)
└── .gitignore
```

Все три YAML не взаимозаменяемы — разные платы, фреймворки, BLE-стеки и компромиссы:

| YAML | Плата | Фреймворк / BLE | Кому |
|---|---|---|---|
| **`esp32-s3-devkitc/atomfast_gateway_s3.yaml`** ⭐ | **ESP32-S3-DevKitC-1 N16R8** (16 MB Flash + 8 MB embedded octal PSRAM, разъём **U.FL/IPEX** для внешней антенны, USB-C) | **esp-idf + Bluedroid** | **Рекомендованный baseline.** Текущая основная конфигурация. Копия `esphome/bluetooth-proxies` upstream, адаптированная под AtomFast: без `bluetooth_proxy:`, с `web_server.log: true` (PSRAM 8 МБ — запас heap есть), switch Народмона `ALWAYS_OFF`. **Wi-Fi + Bluetooth стабильны** — подтверждено **приборным прогоном на самом AtomFast** (2026-06-25): ≈ 96 ч (≈ 4 суток) непрерывно, 0 ребутов, BLE-сессии многочасовые, `web_server` без `json:111` при `log:true`. INC-09/12 на S3 не воспроизводятся. Лог: [`references/run-logs/atomfast-s3-soak-2026-06-25.md`](references/run-logs/atomfast-s3-soak-2026-06-25.md). |
| **`esp32-classic/atomfast_gateway.yaml`** | ESP32-DevKitC v4 (WROOM-32, 4 MB flash, без PSRAM) | **arduino + Bluedroid** | **Архивная сборка** для старых плат, которые уже есть на руках. Прошла многочасовые прогоны на arduino без регрессий, но на esp-idf нестабильна — **классический DevKitC + esp-idf не использовать**. На новую установку лучше брать S3 N16R8. |
| **`esp32-c3-supermini/atomfast_gateway_c3.yaml`** | **ESP32-C3 SuperMini** (4 MB Flash, 400 КБ SRAM, **без PSRAM**, ≈ 22×18 мм, ~150 ₽) | **arduino + NimBLE** | **Mini-baseline.** Compile PASS, **smoke у клиента pending**. Flash занят на **92.3 %** (1.69 МБ из 1.83 МБ) — узкий запас 142 KB на дальнейший рост Web UI. `web_server.log: false` (без PSRAM иначе OOM), `scan_parameters: 1024/16` (duty 1.5 % — single-band coex), без `sg_re`/`raw_atomfast`, без `bluetooth_proxy`. Для носимых / mini-шлюзов где габариты критичнее запаса памяти. |

**Рекомендация: `esp32-s3-devkitc/atomfast_gateway_s3.yaml` на ESP32-S3-DevKitC-1 N16R8.** Стабильная, актуальная, с запасом по памяти и внешней антенной.

## Что это решает

Приложение AtomFast красивое, но:
- **закрытое** — нет API, данные живут только в телефоне;
...
esphome run --device COM3 esp32-classic/atomfast_gateway.yaml
...
```bash
esphome run --device atomfast-gw-s3.local esp32-s3-devkitc/atomfast_gateway_s3.yaml   # S3 (рекомендуется)
esphome run --device atomfast-gw.local    esp32-classic/atomfast_gateway.yaml         # classic
esphome run --device atomfast-gw-c3.local esp32-c3-supermini/atomfast_gateway_c3.yaml # C3 SuperMini
```
...
- ⭐ **ESP32-S3-DevKitC-1 N16R8** (16 МБ Flash + 8 МБ embedded octal PSRAM, U.FL/IPEX, USB-C) → `esp32-s3-devkitc/atomfast_gateway_s3.yaml` (esp-idf, 640/32, log:true). **Рекомендованная актуальная конфигурация.** Внешняя антенна через IPEX повышает дальность BLE до прибора, embedded octal PSRAM решает INC-05, S3-чип развязывает coex. **Подтверждено приборным прогоном на самом AtomFast** (2026-06-25): живая S3-плата отработала **≈ 96 ч (≈ 4 суток) непрерывно, 0 ребутов**, BLE-сессии многочасовые (последняя — 5.6 ч на момент съёмки, реконнектов всего 12 за 4 суток), все сенсоры публикуют, `web_server` без `json:111` при `log:true`. Лог прогона: [`references/run-logs/atomfast-s3-soak-2026-06-25.md`](references/run-logs/atomfast-s3-soak-2026-06-25.md). Это снимает прежнюю оговорку «перенос гипотезы с Radex-S3» — тезис «S3 решает INC-09/12» теперь подтверждён на AtomFast напрямую.
- **Классический ESP32-DevKitC v4** (WROOM-32, 4 MB flash, без PSRAM) → `esp32-classic/atomfast_gateway.yaml` (arduino, 600/180, log:false). Работает только на arduino-baseline. С esp-idf на этой плате — INC-09 (rsn 0x3e, медленные коннекты) и INC-12 (Auth Expired). Имеет смысл, только если плата уже есть на руках и не хочется покупать S3.
- **ESP32-C3 SuperMini** (4 МБ Flash, 400 КБ SRAM, без PSRAM, single-band 2.4 ГГц, ≈ 22×18 мм) → `esp32-c3-supermini/atomfast_gateway_c3.yaml` (arduino + NimBLE, 1024/16, log:false). **Compile PASS, smoke у клиента pending.** Flash занят на 92.3 % (узкий запас 142 KB) — рост Web UI требует кастомный partition CSV. Подходит для носимых / mini-шлюзов где габариты критичнее запаса памяти. Не пытайся прошить S3 YAML на C3 — INC-C3-001 (ENOSPC, 16 MB partition на 4 MB Flash).
...
- **AtomFast** ( [`КБ Радар`](https://kbradar.org/)) — отличный карманный дозиметр.
